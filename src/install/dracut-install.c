...
/* Expands the R(UN)PATH of the ELF header map and searches it for a library
...
\
        for (int i = 0; i < ELF_BYTESWAP(16, ehdr->e_shnum); i++) { \
...
static void resolve_deps_dlopen_parse_json(Hashmap *pdeps, Hashmap *deps, const char *fullsrcpath, size_t src_len,
                                           const char *json, int note_idx, const Elf64_Ehdr *match64, const Elf32_Ehdr *match32)
{
        ...
        if (sd_json_parse(json, 0, &dlopen_json, NULL, NULL) != 0 || !sd_json_variant_is_array(dlopen_json)) {
                log_warning("WARNING: .note.dlopen entry #%d is not a JSON array in '%s'", note_idx, fullsrcpath);
                return;
        }

        for (int entry_idx = 0; entry_idx < sd_json_variant_elements(dlopen_json); entry_idx++) {
                ...
                if (!sonames || !sd_json_variant_is_array(sonames)) {
                        log_warning("WARNING: soname array missing from .note.dlopen entry #%d.%d in '%s'", note_idx, entry_idx, fullsrcpath);
                        return;
                }

                for (int soname_idx = 0; soname_idx < sd_json_variant_elements(sonames); soname_idx++) {
                        sd_json_variant *soname_json = sd_json_variant_by_index(sonames, soname_idx);
                        if (!sd_json_variant_is_string(soname_json)) {
                                log_warning("WARNING: soname #%d of .note.dlopen entry #%d.%d is not a string in '%s'", soname_idx, note_idx, entry_idx,
                                            fullsrcpath);
                                return;
                        }
...
#define RESOLVE_DEPS_NEEDED_FOR_BITS(B, match64, match32) do { \
        PARSE_ELF_START(B, map); \
\
        /* Install the ELF interpreter, if present */ \
        if (ELF_BYTESWAP(16, ehdr->e_type) == ET_EXEC || ELF_BYTESWAP(16, ehdr->e_type) == ET_DYN) { \
                for (int ph_idx = 0; ph_idx < ELF_BYTESWAP(16, ehdr->e_phnum); ph_idx++) { \
...
static int dracut_install(const char *orig_src, const char *orig_dst, bool isdir, bool resolvedeps, bool hashdst)
{
        ...
        now to avoid dependency loops. If it fails once, it will probably fail
        every time anyway. */
